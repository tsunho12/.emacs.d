...
(2) Then, they need to make sure that \\begin{document} and end{document} exist in the current buffer.
(2) Also, they need to make sure that the pdf output of the current buffer exists (should have the same name. i.e. XXX.tex <=> XXX.pdf).
(3) Finally, they need to make sure that the document where want to add the note is the format document Skim.

If any of conditions mentioned above is not satisfied, the result may be unexpected."
  (interactive
...
(add-hook 'LaTex-mode-hook
       (lambda()
         (add-to-list 'TeX-command-list '("XeLaTeX" "%s'xelatex --synctex=1%(mode)%' %t" TeX-run-TeX nil t))
         ...
         (add-to-list 'TeX-command-list '("LuaLaTeX" "%s'LuaLaTeX --synctex=1%(mode)%' %t" TeX-run-TeX nil t))))
...
(custom-set-variables
 '(TeX-source-correlation-method 'synctex)
 '(TeX-source-correlation-mode t)
 '(TeX-source-correlation-start-server t))
(server-force-delete)

;; Auto-raise Emacs on activation
;; (defun raise-emacs-on-aqua()
;; (shell-command "osascript -e 'tell application \"Emacs\" to activate' &"))
;; (add-hook 'server-switch-hook 'raise-emacs-on-auqa)
