...
on select_document(filename)
	tell application "Skim"
		documents where name contains filename
...
-- new_anchor_note: 在Skim中新建一个锚点笔记
-- @param: string title, 笔记标题
-- @param: string content, 笔记文本内容
-- [@param: the clipboard, 笔记图片内容]
on new_anchor_note(note_title, note_content)
...
-- argv 1: title
-- argv 2: filename of content file (txt)
on run argv
	local title, content
	set title to item 1 of argv
	set content to read (POSIX file ((do shell script "pwd .") & "/" & (item 2 of argv))) as «class utf8»
	new_anchor_note(title, content)
end run